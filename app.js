	...
	methods:{
		...
		sendChat:function(e){
			...
			if (this.typing.length) {
				...
				this.chats.push(chatM);
				Object.keys(dataChannels).map((peer_id)=>dataChannels[peer_id].send(JSON.stringify(chatM)));
				this.typing="";
				composeElement.textContent="";
				composeElement.blur;
			}
		},
		handleIncomingDataChannelMessage:function(chatM) {
			switch(chatM.type){
				case "chat":
					this.showChat=true;
